...
use crate::{utils::PKG_PROJECT_ROOT, Result};
...
impl Target {
    ...
    pub(crate) fn get_bin_paths(&self, bin_name: &str, release: bool) -> Vec<Utf8PathBuf> {
        ...
        for target in self.get_cargo_targets() {
            ...
            let bin_path = mode_dir.join(format!("{}{}", bin_name, consts::EXE_SUFFIX));
